...
const SimpleHODDashboard = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/leave-applications?status=advisor_approved,hod_approved,hod_rejected`, {
        headers: {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      advisor_approved: { label: 'Advisor Approved', className: 'bg-blue-100 text-blue-800' },
      hod_approved: { label: 'Approved by You', className: 'bg-green-100 text-green-800' },
      hod_rejected: { label: 'Rejected by You', className: 'bg-red-100 text-red-800' },
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-secondary/20">
      <header className="bg-card border-b shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-primary">Sri Eshwar HOD Portal</h1>
...
export default SimpleHODDashboard;
